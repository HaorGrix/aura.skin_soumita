      ...
      {open && (
        <div className="fixed inset-0 z-[var(--z-modal)] flex items-end justify-center p-0 sm:items-center sm:p-6">
          ...
          <motion.div
            ...
            className="relative z-10 w-full max-w-md overflow-hidden rounded-t-[1.75rem] bg-white p-6 text-center shadow-lift ring-1 ring-line sm:rounded-[1.75rem] sm:p-8"
          >
            <button
              onClick={onClose}
              aria-label="Close"
              ...
              {!sent ? (
                <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <motion.span
                    ...
                    className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-petal text-magenta shadow-[var(--shadow-glow-pink)]"
                  >
                  ...
                  <p className="mx-auto mt-3 max-w-sm text-sm leading-relaxed text-ink-soft">
                    We’ll notify you the moment <span className="font-medium text-ink">{product.name}</span> is
                ...
                <motion.div key="done" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="py-4">
                  ...
                  <h2 className="mt-4 font-serif text-2xl text-ink">You’re on the list! 🎉</h2>
                  <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-ink-soft">
                    We’ll ping <span className="font-medium text-ink">{email}</span> the second{" "}
