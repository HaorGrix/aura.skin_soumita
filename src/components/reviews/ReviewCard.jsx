    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-ink-soft">{whenLabel(r.daysAgo)}</span>
          </div>
        </div>
      </div>

      {r.title && <p className="mt-3 font-semibold text-ink">{r.title}</p>}
      {r.body && <p className="mt-1 leading-relaxed text-ink-soft">{r.body}</p>}
      ...
      {r.hasPhoto && (
        <div className="mt-3 flex gap-2">
          {[0, 1].map((i) => (
            <span
              key={i}
              className="h-16 w-16 rounded-xl ring-1 ring-line"
      ...
      <button
        ...
        className={`mt-3 inline-flex items-center gap-1.5 text-xs font-medium transition-colors ${
          voted ? "text-magenta" : "text-ink-soft hover:text-magenta"
        }`}
      >
